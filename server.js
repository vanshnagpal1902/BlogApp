...
app.get("/updatepost/:title", login, async (req, res) => {
    ...
    console.log(posts[0].filepaths);
    res.render("update", { posts });
});

app.post("/updatepost", upload.single("image"), login, async (req, res) => {
    const { post, content, old } = req.body;
    const newImage = req.file; // Get the uploaded image file

    try {
        // Find the post by the old title
        ...
        existingPost.title = post;
        existingPost.content = content;

        // Update the image if a new one was uploaded
        if (newImage) {
            // Optionally, delete the old image from the server
            existingPost.filepaths.forEach(filepath => {
                fs.unlinkSync(path.join(__dirname, "upload", filepath)); // Delete old image files
            });

            existingPost.filepaths = [newImage.filename]; // Save the new image path
        }
